...
export default function RegistreSeClean() {
  ...
  const handleRegistrar = async () => {
    ...
    try {
      const data = await signUp({ email, senha, papel: 'USER' });
      if (data) {
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#D3D3D3', padding: 20 },
  header: { flexDirection: 'row', alignItems: 'center', marginBottom: 30, marginTop: 40 },
  voltar: { fontSize: 30, marginRight: 10, fontWeight: 'bold' },
  titulo: { fontSize: 24, fontWeight: 'bold', color: '#000' },
  input: { backgroundColor: '#fff', borderRadius: 25, paddingHorizontal: 20, paddingVertical: 15, borderWidth: 1, borderColor: 'gray', marginBottom: 15, fontSize: 16 },
  botao: { backgroundColor: '#000', padding: 15, borderRadius: 30, alignItems: 'center', marginTop: 20 },
  textoBotao: { color: '#fff', fontSize: 18, fontWeight: 'bold' },
  botaoSecundario: { backgroundColor: 'transparent', padding: 15, alignItems: 'center', marginTop: 10 },
  textoBotaoSecundario: { color: '#000', fontSize: 16, fontWeight: 'bold', textDecorationLine: 'underline' },
});